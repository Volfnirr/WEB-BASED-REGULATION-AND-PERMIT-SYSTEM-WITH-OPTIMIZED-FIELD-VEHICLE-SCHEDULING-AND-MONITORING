...
export default function ServicesPage() {
  ...
  const services = [
    { 
      ...
      hasSubMenu: false,
      description: 'To apply for an Agricultural Free Patent (RO-L-03), submit an application form, joint witness affidavit, tax declaration, proof of ownership, approved survey plan, and court clearance proving the land is alienable and undisputed.' 
    },
    { 
      title: 'Issuance of Tree Cutting Permit (RO-F-06)', 
      hasSubMenu: false,
      description: 'To secure a Tree Cutting Permit (RO-F-06), you must submit a formal application letter, proof of land ownership (such as a land title), clear photographs of the specific trees, an official LGU Barangay/Municipal Endorsement or Certification of No Objection, and a technical inventory report alongside applicable processing and inspection fees.' 
    },
    { 
      title: 'Application for Chainsaw Registration (RO-F-04)', 
      hasSubMenu: false,
      description: 'To register a chainsaw under Application for Chainsaw Registration (RO-F-04), you must submit a completed application form, proof of ownership (such as a sales invoice or deed of sale), detailed technical specifications, an official stencil of the chainsaw’s serial number, a legal purpose permit or barangay certification, and the PHP 500.00 registration fee while presenting the actual unit for verification.' 
    },
    { 
                    ...
                    <div className="space-y-4 mt-2 border-t border-gray-100 pt-4">
                      ...
                      {service.hasSubMenu ? (
                        service.subItems.map((subItem, subIndex) => (
                          <div key={subIndex} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div className="flex-1">
                              <span className="text-gray-800 font-medium text-base">{subItem.name}</span>
                              
                              <p className="text-gray-600 text-sm mt-1">{subItem.description}</p>
                            </div>
